...
export default function AdminDashboard() {
  ...
  const fetchDashboardData = async () => {
    ...
    try {
      ...
      if (lowStockRes.data && (lowStockRes.data as any).success) {
        setLowStockProducts((lowStockRes.data as any).data);
      }
    } catch (error) {
  ...
  const statCards = [
    {
      ...
      change: t("dashboard.stats.revenue_summary"),
      trend: "up",
      icon: DollarSign,
    },
    {
      ...
      change: t("dashboard.stats.pending_orders", { count: stats?.orders?.pending || 0 }),
      trend: "up",
      icon: ShoppingCart,
    },
    {
      ...
      change: t("dashboard.stats.new_today", { count: stats?.users?.today || 0 }),
      trend: "up",
      icon: Users,
    },
    {
  ...
  return (
    <div className="space-y-8 relative overflow-hidden">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-4xl font-black text-white tracking-tight uppercase">{t("dashboard.title")}</h1>
          <p className="text-gray-400 mt-1">{t("dashboard.subtitle")}</p>
        </div>
        <div className="flex items-center gap-3">
          <Badge className="bg-red-500/10 text-red-500 border-red-500/20 px-4 py-1.5 font-bold uppercase tracking-widest text-[10px]">
          ...
          <Card className="p-8 border-white/5 bg-white/2 backdrop-blur-sm shadow-2xl h-full relative overflow-hidden">
            ...
            <div className="flex items-center justify-between mb-10">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h2 className="text-xl font-black text-white uppercase tracking-tight">{t("dashboard.recent_orders.title")}</h2>
                  <p className="text-[10px] text-gray-500 uppercase tracking-widest font-bold">{t("dashboard.recent_orders.subtitle")}</p>
                </div>
              </div>
            ...
            <div className="space-y-4">
              {recentOrders.length === 0 ? (
                <div className="text-center py-20">
                  ...
                  <p className="text-gray-500 font-bold uppercase tracking-widest text-[10px]">{t("dashboard.recent_orders.no_orders")}</p>
                </div>
              ) : (
                  ...
                  >
                    ...
                    >
                      ...
                      <div className="flex items-center gap-5 relative z-10">
                        ...
                        <div>
                          <p className="font-bold text-white group-hover/item:text-red-400 transition-colors truncate max-w-[120px] sm:max-w-none">
                            {order.items?.[0]?.product?.name || "ไม่ทราบชื่อสินค้า"}
                            {order.items?.length > 1 && ` และอีก ${Math.max(0, order.items.length - 1)} รายการ`}
                          </p>
                          <p className="text-[10px] text-gray-500 font-black uppercase tracking-tighter mt-1 opacity-60">
                            {order.user?.username || "Guest"} <span className="mx-1">•</span> #{order.id.substring(0, 8)}
                          </p>
                        </div>
                      </div>
                      <div className="text-right relative z-10">
                        ...
                        >
                          {order.status === "COMPLETED" ? t("dashboard.recent_orders.status_completed") : order.status}
                        </Badge>
                      </div>
                    </div>
                  </motion.div>
            ...
            <Card className="p-8 border-white/5 bg-white/2 backdrop-blur-sm shadow-2xl relative overflow-hidden">
              ...
              <div className="flex items-center justify-between mb-10">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h2 className="text-xl font-black text-white uppercase tracking-tight">{t("dashboard.best_sellers.title")}</h2>
                    <p className="text-[10px] text-gray-500 uppercase tracking-widest font-bold">{t("dashboard.best_sellers.subtitle")}</p>
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-4">
                {stats?.topProducts?.length === 0 ? (
                  <div className="text-center py-10 text-gray-500 font-bold uppercase tracking-widest text-[10px]">{t("dashboard.best_sellers.no_data")}</div>
                ) : (
                  stats?.topProducts?.map((product: any) => (
                    <div key={product.id} className="flex items-center justify-between p-4 rounded-2xl bg-white/5 border border-white/5 group hover:bg-white/10 transition-all">
                      <div className="flex items-center gap-4">
                        <div className="w-10 h-10 rounded-xl bg-white/5 flex flex-col items-center justify-center overflow-hidden border border-white/10 group-hover:border-red-500/30 transition-colors shadow-inner">
                          {product.image ? (
                        ...
                        <div>
                          ...
                          <p className="text-[9px] text-gray-500 font-black uppercase tracking-widest">{product.sales} {t("dashboard.best_sellers.sales")}</p>
                        </div>
                      </div>
              ...
              <div className="mt-8 pt-6 border-t border-white/5">
                <div className="grid grid-cols-2 gap-4">
                  {[
                    { label: t("dashboard.quick_links.products"), href: "/admin/products", icon: Package },
                    { label: t("dashboard.quick_links.orders"), href: "/admin/orders", icon: ShoppingCart },
                    { label: t("dashboard.quick_links.users"), href: "/admin/users", icon: Users },
                    { label: t("dashboard.quick_links.licenses"), href: "/admin/licenses", icon: Key },
                  ].map((link) => (
              ...
              >
                <Card className="p-8 border-red-500/20 bg-red-500/5 backdrop-blur-sm shadow-2xl relative overflow-hidden">
                  ...
                  <div className="flex items-center gap-4 mb-6">
                    ...
                    <div>
                      <h2 className="text-lg font-black text-white uppercase tracking-tight">{t("dashboard.low_stock.title")}</h2>
                      <p className="text-[9px] text-red-500/70 uppercase tracking-widest font-black">{t("dashboard.low_stock.subtitle")}</p>
                    </div>
                  </div>
                    ...
                    {lowStockProducts.map((product) => (
                      <div key={product.id} className="flex items-center justify-between p-4 rounded-xl bg-white/5 border border-white/5 group hover:bg-white/10 transition-all">
                        ...
                        <Badge className="bg-red-500 text-white border-none font-black text-[10px] px-3 py-1 rounded-lg">
                          {t("dashboard.low_stock.remaining", { count: product.stock })}
                        </Badge>
                      </div>
                    ))}
                  </div>
